...
CMS_trigger_{codename[0]}       lnN     1.01               """.format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)
    ...
    for j in range(0,Nbkg_int):
        for i in range(0,len(background_shapeSyst[j])):
            card += """
{background_shapeSyst} shapeN2  """.format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs,i=i,background_shapeSyst=background_shapeSyst[j][i])
...
sigXSsyst           lnN     1.034          """
    ...
    if (doSignalShape_unc):
        for i in range(0,len(signal_shapeSyst)):
            card += """
{signal_shapeSyst}        shapeN2  1.0          """.format(signal_shapeSyst=signal_shapeSyst[i])
