...
lumi_7TeV       \t    lnN \t    1.022             """
    ...
    if (codename=="ch7" or codename=="ch8" or codename=="ch10"):
        sherpa_down_=1.-(1.-sherpa_down[k])*0.5
        sherpa_up_=(sherpa_up[k]-1.)*0.5+1.
        scale_down_=1.-(1.-scale_down[k])*0.5
        scale_up_=(scale_up[k]-1.)*0.5+1.
    elif (codename=="ch9"):
#        sherpa_down_=1.-(1.-sherpa_down[k])*2
#        sherpa_up_=(sherpa_up[k]-1.)*2+1.
        sherpa_down_=(sherpa_down[k]-1.)*0.5+1.
        sherpa_up_=1.-(1.-sherpa_up[k])*0.5
        scale_down_=1.-(1.-scale_down[k])*0.5
        scale_up_=(scale_up[k]-1.)*0.5+1.
    else:
        sherpa_down_=sherpa_down[k]
        sherpa_up_=sherpa_up[k]
        scale_down_=scale_down[k]
        scale_up_=scale_up[k]
...
# set sherpa and scale uncertainty to 10%
    card += """       
Sherpa           lnN     {down}/{up}          """.format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg[i],norm_obs=norm_obs, down=sherpa_down_,up=sherpa_up_)

    for i in range(0,Nbkg_int):
        card += """\t\t\t\t-""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)

    card += """       
Scale           lnN     {down}/{up}          """.format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg[i],norm_obs=norm_obs,down=scale_down_,up=scale_up_)
