...
# set sherpa and scale uncertainty to 10%
    card += """       
Sherpa           lnN     {down}/{up}          """.format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg[i],norm_obs=norm_obs, down=sherpa_down[k],up=sherpa_up[k])

    for i in range(0,Nbkg_int):
        card += """\t\t\t\t-""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)

    card += """       
Scale           lnN     {down}/{up}          """.format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg[i],norm_obs=norm_obs,down=scale_down[k],up=scale_up[k])
