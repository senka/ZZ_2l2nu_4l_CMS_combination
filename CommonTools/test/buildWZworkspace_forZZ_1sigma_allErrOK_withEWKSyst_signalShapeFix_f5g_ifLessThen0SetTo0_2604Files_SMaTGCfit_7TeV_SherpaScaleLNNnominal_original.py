...
#            theta[i] = theWS.var("%s"%signal_shapeSyst[i])
    ...
    aTGCPdf = RooATGCSemiAnalyticPdf_wz('ATGCPdf_WWgammaZ_WV_atgc_semileptonic_%s'%codename,
                                        ...
                                        dg1,
                                        dibosonPdf,
                                        '%s/signal_WV_%s_f5g_ifLessThen0SetTo0_2604Files_SMaTGCfit.root'%(basepath,codename),
                                        limtype
                                        )
...
lumi_7TeV       \t    lnN \t    1.022             """
    for i in range(0,Nbkg_int):
        if (i==2): card += """\t\t\t\t1.022""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)
        else: card += """\t\t\t\t-""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)

# set sherpa and scale uncertainty to 10%
    if (codename=="ch7"):
        card += """       
Sherpa           lnN     0.8741/1.1259          """.format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg[i],norm_obs=norm_obs)
    elif (codename=="ch8"):
        card += """       
Sherpa           lnN     0.9878/1.012          """.format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg[i],norm_obs=norm_obs)
    elif (codename=="ch9"):
        card += """       
Sherpa           lnN     1.0855/0.9145          """.format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg[i],norm_obs=norm_obs)
    elif (codename=="ch10"):
        card += """       
Sherpa           lnN     0.9773/1.0227          """.format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg[i],norm_obs=norm_obs)
    elif (codename=="ch11"):
        card += """       
Sherpa           lnN     0.7354/1.2646          """.format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg[i],norm_obs=norm_obs)
    elif (codename=="ch12"):
        card += """       
Sherpa           lnN     0.3743/1.626          """.format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg[i],norm_obs=norm_obs)
    ...
    if (codename=="ch7"):
        card += """       
Scale           lnN     0.7055/1.2945          """.format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg[i],norm_obs=norm_obs)
    elif (codename=="ch8"):
        card += """       
Scale           lnN     0.8535/1.1465          """.format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg[i],norm_obs=norm_obs)
    elif (codename=="ch9"):
        card += """       
Scale           lnN     0.9572/1.0428          """.format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg[i],norm_obs=norm_obs)
    elif (codename=="ch10"):
        card += """       
Scale           lnN     0.8911/1.1089          """.format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg[i],norm_obs=norm_obs)
    elif (codename=="ch11"):
        card += """       
Scale           lnN     0.7065/1.2935          """.format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg[i],norm_obs=norm_obs)
    elif (codename=="ch12"):
        card += """       
Scale           lnN     0.1461/1.8539          """.format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg[i],norm_obs=norm_obs)

    for i in range(0,Nbkg_int):
        if (i==2): card += """\t\t\t\t0.941/1.059""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)
        else: card += """\t\t\t\t-""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)
...
CMS_eff_{codename[0]}           lnN     1.03          """.format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg[i],norm_obs=norm_obs)
    ...
    card += """       
CMS_hzz2l2v_sys_topwwwjetsdata_7TeV_{codename[0]}       lnN     -               """.format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)
...
#        else: card += """\t\t\t\t-""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)
    ...
    card += """
Zlldata_syst_7TeV           lnN     -          """
    for i in range(0,Nbkg_int):
        if (i==0): card += """\t\t\t\t1.25""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)
        else: card += """\t\t\t\t-""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)

    card += """
pdf_VV           lnN     1.036          """
    for i in range(0,Nbkg_int):
        if (i==2): card += """\t\t\t\t1.031""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)
        else: card += """\t\t\t\t-""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)
