...
#    print '\t ----> isItCorrelated: testing ',name
    if ('_eff_b' in name or '_les' in name or '_pu' in name or '_umet' in name or '_res_j' in name or '_scale_j' in name or '_ewk' in name or '_sherpa' in name or '_QCD_ACC_JVeto' in name or 'CMS_hzz2l2v_stat_ll_zz2l2nu_8TeV' in name):
#        print '-> true'
        return True
    else:
#        print '-> false'
...
for section in fit_sections:
    ...
    lType = codename
    print '\n\tlType=',lType
    f = TFile('%s/%s_assm_boosted.root'%(basepath,codename))

    print '****************************************************** using file: %s/%s_assm_boosted.root **************'%(basepath,codename)
