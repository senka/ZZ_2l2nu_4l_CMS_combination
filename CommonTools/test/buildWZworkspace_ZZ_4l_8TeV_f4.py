...
#    lType = section
    lType = codename
    print 'lType=',lType
    f = TFile('%s/%s_ZZ_4l_8TeV_boosted.root'%(basepath,codename))
    ...
    aTGC = RooATGCFunction_wz('ATGC_shapescale_WWgammaZ_WV_atgc_semileptonic_%s'%codename,
                              ...
                              dkg,
                              dg1, 
                              '%s/signal_%s_ZZ_4l_8TeV_f4.root'%(basepath,codename))
...
------------
lumi_8TeV           lnN     1.022		      -""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)
...
{background_shapeSyst} shape1  -                         1.0 """.format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs,i=i,background_shapeSyst=background_shapeSyst[i])
    card += """
sigXSsyst           lnN     1.12                     -""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs,i=i)
    if (doSignalShape_unc):
        for i in range(0,len(signal_shapeSyst)):
            card += """
