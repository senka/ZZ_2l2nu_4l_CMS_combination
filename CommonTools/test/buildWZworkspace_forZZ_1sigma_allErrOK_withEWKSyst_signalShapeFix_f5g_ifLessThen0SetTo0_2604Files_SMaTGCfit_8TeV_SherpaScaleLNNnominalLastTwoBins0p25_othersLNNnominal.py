...
sherpa_up = [1.386,1.026,1.010, 1.003,0.751, 0.431]
sherpa_down = [0.614,0.974,0.990, 0.997,1.249, 1.569]
scale_up = [2.047,1.291,1.064,1.114,1.294,1.847]
scale_down = [0.001,0.709,0.934,0.886,0.706,0.153]
...
lumi_8TeV       \t    lnN \t    1.026             """
    ...
    if (codename=="ch5" or codename=="ch6"):
#        sherpa_down_=1.-(1.-sherpa_down[k])*3.
#        sherpa_up_=(sherpa_up[k]-1.)*3.+1.

        sherpa_down_=(sherpa_down[k]-1.)*0.25+1.
        sherpa_up_=1.-(1.-sherpa_up[k])*0.25
        scale_down_=1.-(1.-scale_down[k])*0.25
        scale_up_=(scale_up[k]-1.)*0.25+1.
        print '==========> sherpaDown ',sherpa_down[k],' -> ',sherpa_down_
        print '==========> sherpaUp ',sherpa_up[k],' -> ',sherpa_up_
    else:
...
Scale           lnN     {down}/{up}          """.format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg[i],norm_obs=norm_obs,down=scale_down_,up=scale_up_)
    ...
    for i in range(0,Nbkg_int):
        if (i==2): card += """\t\t\t\t0.946/1.054""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)
        else: card += """\t\t\t\t-""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)
