...
for section in fit_sections:
    ...
    for j in range(0,Nbkg_int):
        ...
        for i in range(0,len(background_shapeSyst[j])):
            ...
            background_backshapeUp_yield[j].append((f.Get('%sUp'%background_shapeSyst[j][i])).Integral())
            background_backshapeDown_yield[j].append((f.Get('%sDown'%background_shapeSyst[j][i])).Integral())
