...
#            theta[i] = theWS.var("%s"%signal_shapeSyst[i])
            ...
            #theWS.factory("%s[-7,7]"%signal_shapeSyst[i])
#            theta[i] = var("%s"%signal_shapeSyst[i])
            theta[i] = RooRealVar("%s[-7,7]"%signal_shapeSyst[i],"%s[-7,7]"%signal_shapeSyst[i],0)
...
------------""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs,Nbkg_int=Nbkg_int)
    for i in range(0,Nbkg_int):
        card += """
shapes WV_semileptonic_bkg{Nbkg_int}_{codename}  {codename}boosted ./{codename}_boosted_ws.root WV_{codename}boosted:$PROCESS""".format(Nbkg_int=i+1,codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)
#        card += """
...
#QCDscale_VV           lnN     -          """
#    for i in range(0,Nbkg_int):
#        if (i==2): card += """\t\t\t\t1.059""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)
#        else: card += """\t\t\t\t-""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)
    ...
    if (doSignalShape_unc):
        for i in range(0,len(signal_shapeSyst)):
            ...
            if (isItCorrelated(str(signal_shapeSyst[i]))):
                ...
                print '@@@@@@@@@@@@@@ testing ',norm_sig_sm_up[i]/norm_sig_sm,' ',norm_sig_sm_down[i]/norm_sig_sm, ' ',name_forCorr
                if ((norm_sig_sm_up[i]/norm_sig_sm)>0.001 and (norm_sig_sm_down[i]/norm_sig_sm)>0.001):
                    print '@@@@@@@@@@@@@@ testing defout'
                    if (name_forCorr=="CMS_hzz2l2v_QCD_ACC_JVeto"):
                        scale_down_=1.-(1.-norm_sig_sm_down[i]/norm_sig_sm)*2
                        scale_up_=(norm_sig_sm_up[i]/norm_sig_sm-1.)*2+1.

                        card += """
{signal_shapeSyst}        lnN   {down}/{up}         """.format(signal_shapeSyst=name_forCorr,up=scale_up_,down=scale_down_)
                    else:
                        card += """
...
{signal_shapeSyst}        lnN   {down}/{up}         """.format(signal_shapeSyst=name_forCorr,up=norm_sig_sm_up[i]/norm_sig_sm,down=0.001)
                for j in range(0,Nbkg_int):
                    ...
                    elif (j==2 and  name_forCorr=="zz2l2nu_CMS_hzz2l2v_QCD_ACC_JVeto"):
                        card += """\t\t\t\t0.941/1.059""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)
                    else:
                        card += """\t\t\t\t-""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)

            else:
                if (norm_sig_sm_up[i]/norm_sig_sm>0.001 and norm_sig_sm_down[i]/norm_sig_sm>0.001):
                    print '==========> testing uncorrelated ', signal_shapeSyst[i]
                    if ((codename=="ch7" or codename=="ch8" or codename=="ch9" or codename=="ch10") and (signal_shapeSyst[i]=="zz2l2nu_CMS_hzz2l2v_QCD_ACC_JVeto" or signal_shapeSyst[i]=="zz2l2nu_CMS_hzz2l2v_sherpa")):
                        scale_down_=1.-(1.-norm_sig_sm_down[i]/norm_sig_sm)*2
                        scale_up_=(norm_sig_sm_up[i]/norm_sig_sm-1.)*2+1.
                    else:
                        scale_down_=norm_sig_sm_down[i]/norm_sig_sm
                        scale_up_=norm_sig_sm_up[i]/norm_sig_sm
                        
                    card += """
{signal_shapeSyst}        lnN   {down}/{up}         """.format(signal_shapeSyst=signal_shapeSyst[i],up=scale_up_,down=scale_down_)    
                elif (norm_sig_sm_up[i]/norm_sig_sm<0.001 ):
                    card += """
...
{signal_shapeSyst}        lnN   {down}/{up}         """.format(signal_shapeSyst=signal_shapeSyst[i],up=norm_sig_sm_up[i]/norm_sig_sm,down=0.001)    
                ...
                for j in range(0,Nbkg_int):
                    print '=======================!!!!!!!!!> ',j, ' ', signal_shapeSyst[i], ' -> ',signal_shapeSyst[i]=="zz2l2nu_CMS_hzz2l2v_QCD_ACC_JVeto"
                    if (j==2 and  signal_shapeSyst[i]=="zz2l2nu_CMS_hzz2l2v_QCD_ACC_JVeto"):
                        card += """\t\t\t\t0.941/1.059""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)
                    else:
                        card += """\t\t\t\t-""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)
