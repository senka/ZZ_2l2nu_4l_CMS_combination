...
#    print '\t ----> isItCorrelated: testing ',name
    if ('_eff_b' in name or '_les' in name or '_pu' in name or '_umet' in name or '_res_j' in name or '_scale_j' in name or '_ewk' in name or '_QCD_ACC_JVeto' in name
        #or 'CMS_hzz2l2v_stat_ll_zz2l2nu_8TeV' in name or 'CMS_hzz2l2v_stat_ll_zz2l2nu_7TeV' in name
        ):
#        print '-> true'
        return True
    else:
...
#            theta[i] = theWS.var("%s"%signal_shapeSyst[i])
    ...
    aTGCPdf = RooATGCSemiAnalyticPdf_wz('ATGCPdf_WWgammaZ_WV_atgc_semileptonic_%s'%codename,
                                        ...
                                        dg1,
                                        dibosonPdf,
                                        '%s/signal_WV_%s_f4z_ifLessThen1SetTo1_2604Files_SMaTGCfit.root'%(basepath,codename),
                                        limtype
                                        )

    print '****************************************************** using file: %s/signal_WV_%s_f4z_ifLessThen1SetTo1_2604Files_SMaTGCfit.root**************'%(basepath,codename)
...
rate                        {norm_sig_sm}\t""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg[i],norm_obs=norm_obs)
    ...
    if ('ch7' in codename or 'ch8' in codename or 'ch9' in codename or 'ch10' in codename or 'ch11' in codename or 'ch12' in codename):
        card += """           
------------
lumi_7TeV       \t    lnN \t    1.022             """
        for i in range(0,Nbkg_int):
...
pdf_VV           lnN     1.036          """
        for i in range(0,Nbkg_int):
            ...
            elif (i==3): card += """\t\t\t\t1.036""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)
            else: card += """\t\t\t\t-""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)
    else:
        card += """
pdf_VV           lnN     1.058          """
