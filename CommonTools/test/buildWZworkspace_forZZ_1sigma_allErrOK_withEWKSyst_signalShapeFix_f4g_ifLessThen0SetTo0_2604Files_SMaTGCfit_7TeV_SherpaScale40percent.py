...
#            theta[i] = theWS.var("%s"%signal_shapeSyst[i])
    ...
    aTGCPdf = RooATGCSemiAnalyticPdf_wz('ATGCPdf_WWgammaZ_WV_atgc_semileptonic_%s'%codename,
                                        ...
                                        dg1,
                                        dibosonPdf,
                                        '%s/signal_WV_%s_f4g_ifLessThen0SetTo0_2604Files_SMaTGCfit.root'%(basepath,codename),
                                        limtype
                                        )
...
# set sherpa and scale uncertainty to 10%
    card += """       
Sherpa           lnN     1.4          """.format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg[i],norm_obs=norm_obs)

    for i in range(0,Nbkg_int):
        card += """\t\t\t\t-""".format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg,norm_obs=norm_obs)

    card += """       
Scale           lnN     1.4          """.format(codename=codename,norm_sig_sm=norm_sig_sm,norm_bkg=norm_bkg[i],norm_obs=norm_obs)
